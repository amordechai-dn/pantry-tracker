...
As you type an item's name in the add/edit form, a debounced dropdown suggests
matching foods (emoji + name) from a **local, offline** database
(`data/foods.js`, **275 bilingual entries**, generated by
`tools/generate-foods.js`). It's keyboard-friendly (↑/↓/Enter/Esc), touch
friendly, and RTL-correct.

- Picking a suggestion fills the name, assigns its **emoji** to the item, and
  prefills the **category**.
- Even without picking, on save the typed name is best-effort matched
  (exact/prefix, EN or HE) to assign an emoji; if there's no match it **falls
  back to the category emoji**, so cards never look broken.
- The chosen emoji is stored on the item (`emoji` field; existing records are
  migrated safely) and shown on the card.
- Search matches **English and Hebrew**; suggestions display in the current
  language (Hebrew name when available, else English). The scanner also assigns
  emojis from this database.
